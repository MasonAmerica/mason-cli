...
from cli.internal.models.apkparsing.axml.types import *
...
class StringBlock(object):
    ...
    def __repr__(self):
        return "<StringPool #strings={}, #styles={}, UTF8={}>".format(self.stringCount,
                                                                      self.styleCount,
                                                                      self.m_isUTF8)
...
class AXMLParser(object):
    ...
    def __init__(self, raw_buff):
        ...
        if self.buff.size() < 8:
            log.error("Filesize is too small to be a valid AXML file! Filesize: {}".format(
                self.buff.size()))
            self._valid = False
            return
        ...
        if self.buff.size() > 0xFFFFFFFF:
            log.error("Filesize is too large to be a valid AXML file! Filesize: {}".format(
                self.buff.size()))
            self._valid = False
            return
    ...
    def _do_next(self):
        ...
        while self._valid:
            ...
            if h.type < RES_XML_FIRST_CHUNK_TYPE or h.type > RES_XML_LAST_CHUNK_TYPE:
                ...
                # subtract them.
                log.error(
                    "Not a XML resource chunk type: 0x{:04x}. Skipping {} bytes".format(h.type,
                                                                                        h.size))
            ...
            if h.type == RES_XML_START_NAMESPACE_TYPE:
                ...
                log.debug(
                    "Start of Namespace mapping: prefix {}: '{}' --> uri {}: '{}'".format(prefix,
                                                                                          s_prefix,
                                                                                          uri,
                                                                                          s_uri))
            ...
            if _type == START_TAG:
                ...
                if comment:
                    if self.root is None:
                        log.warning("Can not attach comment with content '{}' without root!".format(
                            comment))
                    else:
                ...
                for i in range(self.axml.getAttributeCount()):
                    ...
                    log.debug(
                        "found an attribute: {}{}='{}'".format(uri, name, value.encode("utf-8")))
    ...
    def __init__(self, raw_buff):
        ...
        while self.buff.get_idx() <= self.header.end - ARSCHeader.SIZE:
            ...
            if res_header.end > self.header.end:
                ...
                log.warning("Invalid chunk found! It is larger than the outer chunk: %s",
                            res_header)
                break
            ...
            if res_header.type == RES_STRING_POOL_TYPE:
                ...
                if len(self.packages) > self.packageCount:
                    raise ResParserError(
                        "Got more packages ({}) than expected ({})".format(len(self.packages),
                                                                           self.packageCount))
                ...
                pc = PackageContext(current_package, self.stringpool_main, mTableStrings,
                                    mKeyStrings)
                ...
                if next_idx != self.buff.tell():
                    ...
                    log.error("next_idx = {}, current buffer position = {}".format(next_idx,
                                                                                   self.buff.tell()))
                    log.error(
                ...
                while self.buff.get_idx() <= res_header.end - ARSCHeader.SIZE:
                    ...
                    if pkg_chunk_header.type == RES_TABLE_TYPE_SPEC_TYPE:
                        ...
                        log.warning("Unknown chunk type encountered inside RES_TABLE_PACKAGE: %s",
                                    pkg_chunk_header)
...
class ARSCComplex(object):
    ...
    def __repr__(self):
        return "<ARSCComplex idx='0x{:08x}' parent='{}' count='{}'>".format(self.start,
                                                                            self.id_parent,
                                                                            self.count)
